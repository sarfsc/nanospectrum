...
bool PluginStorage::loadPlugin(const QFileInfo& fileName)
{
    ...
    typedef nanospectrum::Plugin* (*nanospectrum_plugin_init)();
    nanospectrum_plugin_init initFunction = reinterpret_cast<nanospectrum_plugin_init>(pluginLibrary->resolve("nanospectrum_plugin_init"));
...
{    
    loadPluginDirectory("/usr/lib/nanospectrum/plugins/");
    loadPluginDirectory("/usr/lib64/nanospectrum/plugins/");
}
